<template>
  <div class="content">
    <div style="margin-bottom: 20px;">
      <el-row type="flex" class="row-bg">
        ...
        <el-col :span="12" style="text-align:right;">
          <el-button @click="addGoods" type="primary" size="small">添加</el-button>
          <el-button @click="delGoods" type="danger" size="small">删除</el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <div class="table">
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div v-if="scope.row !== null">
              {{Number(scope.row.taxRate).mul(100) + '%'}}
            </div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div v-if="scope.row !== null">
              {{scope.row.preferential === 1 ? '使用' : '不使用'}}
            </div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          width="100">
          <template slot-scope="scope">
            <div v-if="scope.row !== null && scope.row.zeroTaxFlag !== ''">
              <el-tag>{{scope.row.zeroTaxFlag | zeroTaxFlagLabel}}</el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template slot-scope="scope">
            <div v-if="scope.row !== null">
              <el-button @click="editGoods(scope.row)" type="success" size="mini">编辑</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <GoodsForm ref="GoodsForm" :success="getList" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      tableHead: state => state.goods.tableHead
    })
  },
  filters: {
    zeroTaxFlagLabel: function (value) {
      const arr = ['正常征税', '免税', '不征收', '普通零税率']
      return arr[value]
    }
  },
  methods: {
    getList () {
      this.tableLoad = true
      this.$store.dispatch('goods/list', this.listData).then((res) => {
    ...
    addGoods () {
      this.$refs['GoodsForm'].show()
    },
    editGoods (data) {
      this.$refs['GoodsForm'].show('edit', data)
    },
    delGoods () {
      if (this.selectData.length === 0) {
        ...
        this.$confirm('此操作将永久删除该商品, 是否继续?', '提示', {
          ...
          this.$store.dispatch('goods/delete', this.selectData).then((res) => {
            if (res.data.status === 1) {
              this.$notify({
                title: '成功',
