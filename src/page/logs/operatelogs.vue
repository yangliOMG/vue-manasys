<template>
  <div class="content">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span v-if="scope.row !== null">{{scope.row.logtime}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="interf"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div v-if="scope.row !== null">
              {{JSON.parse(scope.row.log).operateName}}
            </div>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template slot-scope="scope">
            <div v-if="scope.row !== null">
              <el-tag v-if="JSON.parse(scope.row.log).status == 1" type="success">成功</el-tag>
              <el-tag v-if="JSON.parse(scope.row.log).status == 0" type="danger">失败</el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column
    ...
    <ViewOperate ref="viewOperate" />
  </div>
</template>
<script>
import { mapState } from 'vuex'
import { table } from '@/mixins/table'
import ViewOperate from '@/components/logs/viewOperate'
export default {
  name: 'operatelogs',
  mixins: [table],
  data () {
  ...
  computed: {
    ...mapState({
      tableHead: state => state.logs.operatehead
    })
  },
  mounted () {
  },
  methods: {
    getList () {
      ...
      this.$store.dispatch('logs/operateList', data).then((res) => {
    ...
    showInfo (data) {
      this.$refs['viewOperate'].show(data)
    }
  },
  components: {
    ViewOperate
